...
> - *RedditSource Adapter* and *SupabaseApiCache* (formerly *SupabaseRedditCache*, renamed once the cache was generalized past Reddit) — the reference implementation of the resilient-source pattern the register mandates for unofficial sources.
>
...
- **Sources:**
  ...
  - Games-industry news coverage — GDELT DOC 2.0 (free, keyless) + curated games-press RSS feeds + Google News RSS as a per-entity thin-coverage fallback, matched to watchlist entities by a standalone news-ingestion module (`agents/workers/news/`) and consumed by this agent as a **fourth, separately-scored source**. News measures media tone/frame (financial performance, controversy, roadmap, competitive position, ...), not player-experience sentiment, so it is deliberately **not** run through the VADER/ABSA/vocal-minority pipeline below — see `docs/news-source-decision-memo.md`. Synthesis keeps it out of the community `avg_score` and instead treats news-vs-community disagreement as its own signal.
...
Claude then deduplicates, maps each game to its studio and parent ticker, and populates the `watchlist` table. The original target was a starting universe of roughly 150–300 games and 30–50 studios without any manual entry; as actually run (seeding plus the RAWG catalog backfill described in `scripts/rawg_backfill.py`, which is broader than the original curated-seed scope), the live watchlist is far larger — **~4,017 active games** as of mid-2026. Each game with an active subreddit also gets a **sentiment tier** (see the RedditSource adapter's operational notes): Tier A titles get full post + comment collection; the long tail gets listing-only coverage so the weekly Reddit budget stays small. At this scale, tier assignment landed very differently than the original design assumed: **3,977 of 4,017 active watchlist rows are `tier_a`** (per CLAUDE.md's "News ingestion internals" section, which cites this figure while explaining why entity-tiering wouldn't shrink the GDELT worker's per-entity query volume) — the reverse of the small-elite-subset/large-tail split the RedditSource adapter's original request-budget math assumed.
...
#### Why Alpaca
Alpaca is a developer-first brokerage with a full REST API purpose-built for algorithmic and agentic trading. Paper trading is free, requires only an email to sign up, starts with a $100K simulated balance, uses real-time IEX market data, and shares the exact same API spec as live trading — the only difference between paper and live is the endpoint and key. No university sponsorship, no scraping, no brittle form automation. Alpaca also ships an **official MCP server** (open-source, maintained by Alpaca, ~60+ tools as of the 2026 v2 rewrite, paper-trading mode on by default) — but as built, the project does **not** use it. The official server exposes a bare `place_order` tool that talks straight to Alpaca's REST API; handing that to an LLM tool-use loop would let a model place an order without the Supabase `status = 'approved'` re-check running, a direct violation of this brief's own safety rules (see the Execution Agent below). Instead, the Agent SDK migration built two custom, minimal in-process MCP servers (`agents/portfolio/alpaca_mcp.py`, via `create_sdk_mcp_server`) that wrap the *existing* `alpaca_trading_client` functions — one read-only server for the Portfolio Manager's account-state lookup, one guarded server for the Execution Agent whose order-placement tool re-reads Supabase status and refuses anything not `approved`. The guard is reused verbatim, never re-implemented, and the official server's broader, ungated tool surface is never exposed to any agent. See CLAUDE.md's "Portfolio agents MCP internals" section for the full detail.
...
| News article stream | GDELT DOC 2.0 (free, keyless) + curated games-press RSS + Google News RSS fallback | Free | 1 by ToS (GDELT/RSS) / 2 (Google News) |
...
- **NewsItems:** url (PK), published_at, domain, title, snippet, matched_entities (jsonb array of `games.game_id`), fetched_at — the shared news-article substrate written once per run by the standalone news-ingestion module (`agents/workers/news/`, migration `008_news_items.sql`), consumed by the Sentiment Agent's `source='news'` rows and available to other workers (Studio Intel, a future Discovery pass) without re-fetching
- **CcuSnapshots:** id, game_id, captured_at, concurrent_players, created_at — raw hourly intraday CCU history collected independently of the weekly pipeline by `scripts/collect_ccu_snapshot.py` (migration `016_ccu_snapshots.sql`); deliberately not merged into `PlayerMetrics`, whose `unique(game_id, date)` constraint and downstream readers assume one row per game per day
...
- **WeeklyBriefings:** id, week_of, briefing_text, portfolio_update, top_opportunities, risk_flags, notable_events, reasoning_log, created_at — the Synthesis Agent's persisted weekly output
...
[ONE-TIME: Watchlist Seeding Agent]
  ...
  Populates watchlist table (150–300 games originally targeted;
  ~4,017 active games live as of mid-2026 once the RAWG backfill ran)
        |
        v
